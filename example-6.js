...
console.log('---- EXAMPLE 6 --- ', 'Put here your function', newObjectResume(companies));
...
function newObjectResume(companies) {
  const data = {};
  for (const company of companies) {
    for (const user of company.users) {
      data[`${(user.lastName) ? user.lastName : ''}${(user.firstName) ? user.firstName : ''}${user.age}`] = user.car;
    }
  }
  return data;
}
